package chain

import "crypto/x509"

// appleRootPool contains Apple's root and intermediate CA certificates
// used for iOS code signing validation.
var appleRootPool *x509.CertPool

func init() {
	appleRootPool = x509.NewCertPool()
	appleRootPool.AppendCertsFromPEM([]byte(appleRootCA))
	appleRootPool.AppendCertsFromPEM([]byte(appleWWDRCAG3))
}

// AppleRootPool returns the certificate pool with Apple root and intermediate CAs.
func AppleRootPool() *x509.CertPool {
	return appleRootPool
}

// Apple Root CA — downloaded from https://www.apple.com/appleca/AppleIncRootCertificate.cer
const appleRootCA = `-----BEGIN CERTIFICATE-----

UKqK1drk/NAJBzewdXUh
-----END CERTIFICATE-----`

// Apple Worldwide Developer Relations Certification Authority G3
// Downloaded from https://www.apple.com/certificateauthority/AppleWWDRCAG3.cer
const appleWWDRCAG3 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
